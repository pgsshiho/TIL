...
그림과 같이 두 역할은 소프트웨어 생명 주기(Life Cycle)의 V 모델에서 각각 좌측과 우측의핵심 역할을 담당하고 서로 보완 관계에 있다  
...
|테스트 방법|설명|테스트 목적|
|----------|---|----------|
|구조 기반|업무 단위별 제어 흐름과 조건 결정에 따른 결과를 테스트하는 데 목적이 있다.  프로그램 내부 구조 및 복잡도를 검증하는 화이트박스테스트가 속한다.|제어 흐름,조건 결정|
|명세 기반|동등 분할과 경계값 분석을 위하여 사용자의 입력,출력,내부 이벤트 등을 확인하는 데 목적이 있음  목적 및 실행 코드 기반의 실행을 통한 블랙박스테스트가 속함|동등 분할,경계값 분석|
