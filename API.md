...
웹 서비스는 일반적으로 비공개이며 승인된 사용자만 액세스가능  
### API, 마이크로서비스 및 클라우드 네이티브 개발
**마이크로 서비스란?**:애플리케이션을 더 작고 독립적인 구성 요소로 나누는 아키텍처 스타일  
종종 REST API를 사용하여 연결  
애플리케이션을 별도의 서비스 모음으로 구축하면 개발자가 하나의 애플리케이션 구성 요소에서 다른 구성 요소와 독립적으로 작업할 수 있으며 애플리케이션을 더 쉽게 테스트, 유지 관리 및 확장 가능  
## API 혜택
새로운 애플리케이션과 서비스의 설계와 개발, 기존 애플리케이션과 서비스의 통합과 관리를 단순화
1. 협업 개선
```API를 사용하면 이러한 플랫폼과 앱이 서로 원활하게 통신할 수 있도록 통합이 가능```
2. 혁신 가속화
   ``` 유연성을 제공하므로 기업은 새로운 비즈니스 파트너와 연결하고 기존 시장에 새로운 서비스를 제공```
3. 데이터 수익화
   ```  API가 귀중한 디지털 자산에 대한 액세스 권한을 부여하면 기업은 액세스 권한을 판매하여 이를 수익화```
4. 시스템 보안
   ``` 요청하는 애플리케이션을 응답 서비스의 인프라와 분리하고 통신할 때 둘 사이에 보안 계층을 제공```
5. 사용자 보안 및 개인정보 보호
   ``` 네트워크 내에서 추가 보호 기능을 제공```
